...
module.exports = class ResourceLoader {
  ...
  createPotentialCORSRequest({ url, destination, corsAttributeState, sameOriginFallback }) {
    let mode = "no-cors";
    let credentialsMode = "include";

    if (corsAttributeState !== null) {
      mode = "cors";
    }
    ...
    return this.fetch({ url, destination, mode, credentialsMode, useURLCredentials: true });
  }
};
...
function HTTPNetworkOrCacheFetch() {
  if (cookieJar && (sameOrigin || mode === "no-cors" || credentialsMode === "include")) {
    requestOptions.jar = wrapCookieJarForRequest(cookieJar);
  }
}

// const { getState: getCORSState } = require("../../living/fetch/cors-settings");
// const corsAttributeState = getCORSState(element.getAttributeNS(null, "crossorigin"));

// let crossOrigin = false;
// if (referrer) {
//   crossOrigin = new URL(referrer).origin !== new URL(urlString).origin;
// }
