    ...
    return Promise.resolve().then(() => {
      ...
      req.catch(err => {
        console.log("=====fetch error", err, url);
          ...
          requestHead("http://web-platform.test:9001/", { strictSSL: false })
          .then(() => {
            console.log(`WPT server at http://web-platform.test:9001/ is up!`);
          })
          .catch(err => {
            console.log(`WPT server at http://web-platform.test:9001/ is not up yet (${err.message})`);
          });

          requestHead('http://web-platform.test:9002/', { strictSSL: false })
          .then(() => {
            console.log(`WPT server at http://web-platform.test:9002/ is up!`);
          })
          .catch(err => {
            console.log(`WPT server at http://web-platform.test:9002/ is not up yet (${err.message})`);
          });
        }
      )
